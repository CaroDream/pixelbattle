...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { data: reservation, error: reservationError } = await supabaseAdmin.rpc('reserve_pixel', {
      ...
      p_y: y,
      p_color: color,
      p_display_text: displayText || 'Anonymous',
    ...
    if (!Number.isInteger(amount) || amount <= 0) throw new Error('Invalid server price');

    const stripeSecret = process.env.STRIPE_SECRET_KEY;
    if (!stripeSecret) throw new Error('Payment is not configured');
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: 'gbp',
          product_data: { name: `Pixel Battle tile (${x}, ${y})` },
          unit_amount: amount,
        },
        quantity: 1,
      }],
    ...
    if (stripe && sessionId) {
      try { await stripe.checkout.sessions.expire(sessionId); } catch {}
    }
    if (orderId) {
      try {
        await supabaseAdmin.from('pixel_orders').update({ status: 'failed' }).eq('id', orderId).eq('status', 'pending');
