...
export async function POST(request: NextRequest) {
  try {
    ...
    if (reservationError || !reservation) {
      return NextResponse.json({ error: reservationError?.message || 'Tile unavailable' }, { status: 409 });
    }

    const orderId = reservation.order_id as string;
    const amount = Number(reservation.amount_gbp_pence);
