...
function sanitizeUrl(raw: unknown): string | null {
  ...
  try {
    ...
    if (/^(127\.|10\.|192\.168\.|169\.254\.)/.test(host) || /^172\.(1[6-9]|2\d|3[01])\./.test(host)) return null;
    return url.href;
  } catch { return null; }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: reservation, error: reservationError } = await supabaseAdmin.rpc('reserve_pixel', {
      p_x: x, p_y: y, p_color: color, p_display_text: displayText || 'Anonymous', p_country_flag: countryFlag, p_social_link: socialLink,
    });
    if (reservationError || !reservation || Array.isArray(reservation)) return NextResponse.json({ error: reservationError?.message || 'Tile unavailable' }, { status: 409 });

    const orderId = (reservation as { order_id: string }).order_id;
    const amount = Number((reservation as { amount_gbp_pence: number }).amount_gbp_pence);
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      line_items: [{ price_data: { currency: 'gbp', product_data: { name: `Pixel Battle tile (${x}, ${y})` }, unit_amount: amount }, quantity: 1 }],
    ...
    return NextResponse.json({ error: 'Payment failed or tile unavailable' }, { status: 500 });
  }
}
